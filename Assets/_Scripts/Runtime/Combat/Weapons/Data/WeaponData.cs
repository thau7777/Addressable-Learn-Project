...
public abstract class WeaponData : ScriptableObject
{
    ...
    [TabGroup("Basic Settings"), SerializeField]
    protected float _baseWeaponRange = 5f;
    ...
    public float BaseWeaponRange => _baseWeaponRange;


    public virtual async UniTask LoadWeaponAssetsAsync()
    {
        if (WeaponPrefab == null && weaponPrefabRef.RuntimeKeyIsValid() && !_weaponAsyncOperationHandle.IsValid())
        {
        ...
        if (WeaponIconPrefab == null && weaponIconRef.RuntimeKeyIsValid() && !_iconAsyncOperationHandle.IsValid())
        {
