...
public class SwingAttackAnimSO : WeaponAttackAnimSO
{
    ...
    public override Sequence Play(Transform weaponModel, Transform target, Vector3 originLocalPos, float range, Quaternion originLocalRot, float attackInterval, Action onSwing, Action onEndSwing)
    {
        ...
        swingSeq = swingSeq.ChainCallback(onSwing)
            .Chain(Tween.LocalRotation(weaponModel, swingRot, scaledSwingDur, _swingEase));

        return Sequence.Create()
            .Group(Tween.LocalPosition(weaponModel, lungePos, scaledLungeDur, _lungeEase))
            .Group(swingSeq)
            .ChainCallback(onEndSwing)
